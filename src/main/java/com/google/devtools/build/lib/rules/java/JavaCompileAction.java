...
public class JavaCompileAction extends AbstractAction {
  ...
  public void execute(ActionExecutionContext actionExecutionContext)
    ...
    try {
      ...
      // Remove the previous jar if it exists.
      Path prevJar = getPreviousJarPath();
      if (prevJar.exists()) {
        try {
          prevJar.delete();
        } catch (IOException e) {
          LoggingUtil.logToRemote(Level.WARNING,
              "Unable to remove jar file '" + prevJar + "'", e);
        }
      }
    }
  }
  ...
  @Override
  public String getMnemonic() { return "Javac"; }
  ...
  private Path getPreviousJarPath() {
    Path outputJar = getOutputJar().getPath();
    String newName = outputJar.getBaseName() + ".old";
    return outputJar.getParentDirectory().getChild(newName);
  }

  @Override
  public void prepare() throws IOException {
    // Rename the existing output jar to improve incremental compilation
    Path outputJar = getOutputJar().getPath();
    if (outputJar.exists()) {
      outputJar.renameTo(getPreviousJarPath());
    }
    super.prepare();
  }
